...
const seedSampleLoans = () => {
  const sampleLoans = [
    ...
    },
    {
      ...
      employmentAddress: "456 Beverly Hills, CA",
      status: "pending",
      createdAt: "2021-06-09"
    },
    {
    ...
    },
    {
      ...
      employmentAddress: "303 Brooklyn, NY",
      status: "approved",
      loanOfficerId: 2,
      createdAt: "2021-06-08"
    },
...
// User Dashboard Statistics to show deficit, approvals, etc.
app.get("/dashboard/user", authenticateToken, async (req, res, next) => {
  try {
    const stats = await new Promise((resolve, reject) => {
      db.get(
        `SELECT 
          ...
          (SELECT SUM(amount) FROM repayments WHERE 
            loanId IN (SELECT id FROM loan_applications WHERE userId = ?) AND 
            status = 'completed') AS totalRepaid
        ...
        (err, row) => {
          if (err) reject(err);
            ...
            // Calculate deficit (what user still owes)
            const totalOwed = row.totalApproved || 0;
            const totalRepaid = row.totalRepaid || 0;
            const deficit = totalOwed - totalRepaid;
...
// Search loans - Used in User Dashboard - Loans
app.get("/loans/search", authenticateToken, async (req, res, next) => {
  try {
    const { query } = req.query;
    let sql, params;
    
    if (req.user.role === 'user') {
      // Regular users can only search their own loans
      sql = `SELECT 
              la.*,
              u.fullName as loanOfficerName
             FROM loan_applications la
             ...
             WHERE la.userId = ? AND 
                  (la.fullName LIKE ? OR 
                   ...
                   la.status LIKE ?)
             ORDER BY la.createdAt DESC`;
      params = [req.user.id, `%${query}%`, `%${query}%`, `%${query}%`, `%${query}%`];
    } else {
      // Admin/verifiers can search all loans
      sql = `SELECT 
              la.*,
